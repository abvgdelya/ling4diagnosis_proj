  ...
  sentences.forEach(sentence => {
    ...
    verbMatches.forEach(verb => {
      ...
      if (position > -1) {
        ...
        if (lowerVerb.match(/^(feel|think|know|believe|seem)$/)) category = 'MENTAL';
        else if (lowerVerb.match(/^(is|are|was|were|be|have|has)$/)) category = 'RELATIONAL';
        else if (lowerVerb.match(/^(say|tell|ask|speak)$/)) category = 'VERBAL';
        else if (lowerVerb.match(/^(laugh|cry|smile|watch)$/)) category = 'BEHAVIORAL';
  ...
  try {
    ...
    if (polarityScore <= 40) {
      return NextResponse.json({
        score: 0, severity: 'Low', presentMarkers: [], 
        ...
        foundMarkers: [], textWithMarkers: text
      });
    }
    ...
    const ellipsisRatio = sentences.filter(s => /\.{2,}/.test(s)).length / Math.max(1, sentences.length) * 100;
    ...
    // Scoring
    const score = Math.round(polarityScore * 0.4 + firstPersonRatio * 0.15 + passiveRatio * 0.15 + semanticRatio * 0.2 + synt1Ratio * 0.05 + ellipsisRatio * 0.05);
    ...
    const result: AnalysisResult = {
      ...
      evaluation: `⚠️ ${severity} RISK (${Math.round(polarityScore)}% sentiment + ${foundMarkers.length} markers)`,
      foundMarkers,
    ...
    return NextResponse.json({ error: "Analysis failed", score: 0, severity: 'Low' }, { status: 500 });
  }
}
